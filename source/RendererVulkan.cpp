...
void RendererVulkan::update(long long dt) {
...
void RendererVulkan::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
...
VkShaderModule RendererVulkan::createShaderModule(const std::vector<char>& code) {
...
void RendererVulkan::createGraphicsPipeline() {
	...
	colorBlendAttatchment.colorBlendOp = VK_BLEND_OP_ADD;
	...
	colorBlendAttatchment.alphaBlendOp = VK_BLEND_OP_ADD;
...
void RendererVulkan::createTextureImage() {
	...
	auto pixels = stbi_load("textures/slay-juice.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
...
void RendererVulkan::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
...
VkFormat RendererVulkan::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
...
bool RendererVulkan::isDeviceSuitable(VkPhysicalDevice device) {
	// In case you want to query for features or fallback to integrated graphicss
...
VKAPI_ATTR VkBool32 VKAPI_CALL RendererVulkan::debugCallback(
	...
	case VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT:
		strcpy_s(message, "Verbose Validation layer: ");
		break;
	case VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT:
		strcpy_s(message, "Info Validation layer: ");
		break;
	case VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT:
		strcpy_s(message, "WARNING Validation layer: ");
		break;
	case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
		strcpy_s(message, "ERROR Validation layer: ");
		break;
	}
...
VkResult RendererVulkan::CreateDebugUtilsMessengerEXT(VkInstance instance,
...
void RendererVulkan::DestroyDebugUtilsMessengerEXT(VkInstance instance,
...
void RendererVulkan::updateUniformBuffer(uint32_t currentImage, uint32_t dt) {
	UniformBufferObject ubo{};
	static float angle = 0;
	float speed = 360.0f;
	angle += (360.0f / 10'000'000.0f) * dt;
	ubo.model = glm::rotate(glm::mat4(1.0f), glm::radians(angle), glm::vec3(0.0f, 0.0f, 1.0f));
...
void RendererVulkan::drawFrame(uint32_t dt) {
...
void RendererVulkan::framebufferResizeCallback(GLFWwindow* window, int width, int height) {
	auto app = reinterpret_cast<RendererVulkan*>(glfwGetWindowUserPointer(window));
